						  ...
						  for (UploadData data : uploadData) {
							  ...
							  if (!isNotValidRow) {
								  ...
								  if (!message.equals( Strings.EMPTY )) {
									...
									if (vaccineService.getVaccineRecord(user.getId(), user.getId(), data.getBulan(), vm.getVaccineCode()) == null) {
					  ...
					  for (UploadData data : uploadData) {
						...
						if (!isNotValidRow) {
							...
							if (!message.equals( Strings.EMPTY )) {
								...
								if ( checkUpService.getCheckUpRecord( user.getId(), child.getId(), cm.getCode()) == null ) {
									CheckUpRequest checkHealth = new CheckUpRequest();
									checkHealth.setUserId( user.getId() );
