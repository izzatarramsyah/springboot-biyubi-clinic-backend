		...
		try {
			...
			if (req.getHeader().getChannel().equals(Constant.CHANNEL_WEB)) {
				if (userAdmin == null) {
					...
					VaccineRecord check = vaccineService.getVaccineRecord(vaccineRecord.getUserId(),
							vaccineRecord.getChildId(),
									...
									if (check == null) {
										if (vaccineService.addVaccineRecord(vaccineRecord)) {
											String value2 = Constant.VALUE_RECORD_VACCINE
													...
													.replaceAll("<batch>", String.valueOf(vaccineRecord.getBatch()))
													.replaceAll("<notes>", vaccineRecord.getNotes());
											logService.saveLog(new Log(Constant.ACTIVITY_ADD_VACCINE_RECORD, req.getHeader().getuName(), value2));
										}
									} else {
									...
									if (check != null) {
										if (vaccineService.updateVaccineRecord(vaccineRecord)) {
											String value2 = Constant.VALUE_RECORD_VACCINE
													...
													.replaceAll("<batch>", String.valueOf(vaccineRecord.getBatch()))
													.replaceAll("<notes>", vaccineRecord.getNotes());
											logService.saveLog(new Log(Constant.ACTIVITY_UPDATE_VACCINE_RECORD, req.getHeader().getuName(), value2));
										}
									} else {
								...
								case Constant.SAVE_CHECKUP_RECORD:
									if (check == null) {
										if (checkUpService.addCheckUpRecord(userAdmin, req.getPayload())) {
											CheckUpRecord record = checkUpService.getCheckUpRecord(user.getId(),
													child.getId(), mstCode);
											String value2 = Constant.VALUE_RECORD_CHECK_UP
													.replaceAll("<childName>", child.getFullname())
													.replaceAll("<weight>", String.valueOf(record.getWeight()))
													.replaceAll("<length>", String.valueOf(record.getLength()))
													.replaceAll("<headDiameter>",String.valueOf(record.getHeadDiameter()));
											logService.saveLog(new Log(Constant.ACTIVITY_ADD_CHECK_UP_RECORD, req.getHeader().getuName(), value2));
										}
									} else {
										statusTrx = StatusCode.RECORD_ALREADY_INSERTED;
									}
									...
									if (check != null) {
										if (checkUpService.updateCheckUpRecord(userAdmin, req.getPayload())) {
											...
											String value2 = Constant.VALUE_RECORD_CHECK_UP
													...
													.replaceAll("<length>", String.valueOf(record.getLength()))
													.replaceAll("<headDiameter>", String.valueOf(record.getHeadDiameter()));
											logService.saveLog(new Log(Constant.ACTIVITY_UPDATE_CHECK_UP_RECORD, req.getHeader().getuName(), value2));
										}
									} else {
										statusTrx = StatusCode.FAILED_PROCESS;
									}
				...
				if (userAdmin == null) {
					...
					switch (req.getHeader().getCommand()) {
						case "info-schedule-vaccine":
							...
							responseObject.put("object", vaccineService.getSchedule(req.getPayload().getUserId(),
									req.getPayload().getChildId()));
							break;
						case "info-schedule-checkup":
							LOG.info("COMMAND : INFO-SCHEDULE-CHECKUP");
							responseObject.put("object", checkUpService.getSchedule(req.getPayload().getUserId(),
									req.getPayload().getChildId()));
							break;
						default:
							break;
