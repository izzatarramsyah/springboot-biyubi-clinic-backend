	...
	public APIResponse<?> login(@RequestBody String input) {
		...
		try {
			...
			if (req.getHeader().getChannel().equals( Constant.CHANNEL_WEB )) {
				...
				UserAdmin userAdmin = userAdminService.isValidUser( req.getPayload().getUsername() , req.getPayload().getPassword() );
				if (userAdmin == null) {
					...
					if (!isValidSession) {
						...
						String sessionId = userAdmin.getSessionId() != null && !"".equalsIgnoreCase(userAdmin.getSessionId())
								? userAdmin.getSessionId() : UUID.randomUUID().toString().replace("-", "").toUpperCase();
		...
		try {
			...
			if (req.getHeader().getChannel().equals( Constant.CHANNEL_WEB )) {
				...
				if (user == null) {
					...
					boolean isValidSession = userAdminService.isValidSession( req.getHeader().getSession() , req.getHeader().getuName());
					if (!isValidSession) {
